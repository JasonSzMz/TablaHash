...
public class Hash extends javax.swing.JFrame {
    ...
    public void getAndSaveInfo(){
        dt.setLimite(Integer.parseInt(txtF_Limit.getText()));
        dt.setName(txtF_Register.getText());
    }
    
    public void getProcess(){
        int x, value=0;
        String textFTA;
        for(x=0; x< dt.getName().length(); x++){
            value = dt.getName().charAt(x);
            textFTA = txtA_Information.getText();
            txtA_Information.setText(textFTA + ("\n\n\t" + dt.getName().charAt(x) + "\tvalor ->\t" + value + "\n"));
        }
